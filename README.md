# Agentive Codex Skills

General-purpose workflow skills for AI-assisted software development in Codex.
...
| **[Architect](architect/)** | `$architect` | Surface architectural friction and design deep module interfaces |
...
This symlinks each skill into `~/.codex/skills`, so edits in this checkout are picked up after Codex reloads skill metadata.

Or symlink individual skills:

```bash
mkdir -p "$HOME/.codex/skills"
ln -sf "$(pwd)/spec" "$HOME/.codex/skills/spec"
ln -sf "$(pwd)/domain" "$HOME/.codex/skills/domain"
```

Then restart Codex or open a new session and invoke them as `$spec`, `$domain`, `$slice`, etc.
